...
When computing source env. cost we have to set a reference point. For example, for oil, we assume it will be burned with 20% oxygen atmosphere, and heat will be dissipated into environment of given temperature. This reference point doesn't matter if our goal is to put pressure on oil-related products alone because we just need to compare them to one another. However if we want to compare oil-related products with products of other branch, then we need to adjust reference points of oil and base resource of the other branch. For example, when selecting electricity supplier we want meaningful costs. Electricity can be produced both using oil and using space for solar cells. This means source cost of space must correspond somewhat to cost of coal.

This correspondence should occur by itself, without any artificial adjustments. However it can be adjusted to promote preserving of selected resources. Also it can by initially out of balance if calculations of source costs are wrong. For example cooling temperature assumption for power plants (18 deg. C vs 25 deg. C) may have large impact on choices between fossil power plants and solar power plants.
